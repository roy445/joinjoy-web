...
export function ShopVisual({ item, large = false }: { item: ShopVisualItem; large?: boolean }) {
  ...
  return <div className={`relative flex items-center justify-center ${large ? "h-[min(62vh,38rem)] min-h-[28rem] w-full" : "h-full min-h-[15rem] w-full"}`}>
    <div className={`absolute ${large ? "inset-12 opacity-80" : "inset-8 opacity-60"} rounded-full ${palette.glow} blur-3xl`} />
...
export function ShopPreviewButton({ item, onAction, actionLabel }: { item: ShopVisualItem; onAction?: () => void; actionLabel?: string }) {
  const [open, setOpen] = useState(false);
  return <><button className="group relative h-full w-full" onClick={() => setOpen(true)} aria-label={`預覽${item.name}`}><ShopVisual item={item} /><span className="absolute bottom-3 right-3 flex items-center gap-1 rounded-full bg-black/35 px-2 py-1 text-[11px] font-bold text-white opacity-0 backdrop-blur transition group-hover:opacity-100"><Maximize2 size={12} /> 全螢幕預覽</span></button><AnimatePresence>{open && <motion.div className="fixed inset-0 z-[80] flex items-center justify-center bg-[#0b1717]/85 p-4 backdrop-blur-xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setOpen(false)}><motion.div className="relative grid max-h-[92vh] w-full max-w-6xl gap-0 overflow-hidden rounded-[32px] bg-[#172a29] text-white shadow-2xl md:grid-cols-[1.35fr_0.65fr]" initial={{ opacity: 0, scale: 0.94, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97 }} onClick={(e) => e.stopPropagation()}><button onClick={() => setOpen(false)} className="absolute right-4 top-4 rounded-full bg-white/10 p-2" aria-label="關閉預覽"><X size={20} /></button><div className="flex min-h-[28rem] items-center justify-center bg-[radial-gradient(circle_at_center,#315d54_0%,#172a29_58%,#101719_100%)] p-6 md:min-h-[38rem]"><ShopVisual item={item} large /></div><div className="flex flex-col justify-center border-t border-white/10 p-6 md:border-l md:border-t-0 md:p-8"><p className="text-xs font-bold uppercase tracking-[0.22em] text-[#9bf8df]">LIVE DECORATION PREVIEW</p><h2 className="mt-3 font-display text-3xl font-black">{item.name}</h2><p className="mt-4 leading-7 text-white/70">{item.description || "這件收藏品擁有獨立的 JoinJoy 視覺效果，點擊預覽可以在兌換前查看完整展示。"}</p><div className="mt-6 flex flex-wrap gap-2"><span className="rounded-full bg-white/10 px-3 py-1.5 text-xs">{item.type === "frame" ? "動態頭像框" : item.type === "title" ? "光效稱號" : "互動徽章"}</span><span className="rounded-full bg-white/10 px-3 py-1.5 text-xs">{item.rarity.toUpperCase()}</span></div>{onAction && <button type="button" onClick={onAction} className="mt-6 w-full rounded-2xl bg-coral-500 px-4 py-3 text-sm font-black text-white shadow-lg shadow-coral-500/20 transition hover:bg-coral-600">{actionLabel || "立即兌換"}</button>}</div></motion.div></motion.div>}</AnimatePresence></>;
}
