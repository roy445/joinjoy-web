    ...
    <div className="min-h-screen bg-[#08131f] text-slate-100">
      <div className="relative overflow-hidden border-b border-cyan-400/15 bg-[radial-gradient(circle_at_80%_0%,rgba(0,220,190,.16),transparent_35%),linear-gradient(135deg,#08131f,#0d2030)]">
        ...
        <div className="mx-auto max-w-6xl px-4 py-12 md:px-8 md:py-20">
          ...
          <p className="mt-5 max-w-2xl text-base leading-7 text-slate-300 md:text-lg">輸入你們的時間、預算與喜好，得到能直接揪團的完整出遊方案。</p><div className="mt-7 grid max-w-2xl grid-cols-3 gap-2 rounded-2xl border border-cyan-300/15 bg-[#07111c]/70 p-2 text-[10px] uppercase tracking-wider text-slate-400 sm:text-xs"><div className="rounded-xl bg-[#58d9ff]/10 px-3 py-2"><span className="mb-1 block h-1.5 w-1.5 rounded-full bg-[#67f5c8]" />條件鎖定</div><div className={`rounded-xl px-3 py-2 ${weatherLoading ? "bg-amber-300/10 text-amber-200" : weather ? "bg-[#67f5c8]/10 text-[#67f5c8]" : "bg-slate-700/30"}`}><span className={`mb-1 block h-1.5 w-1.5 rounded-full ${weatherLoading ? "bg-amber-300" : weather ? "bg-[#67f5c8]" : "bg-slate-500"}`} />{weatherLoading ? "天氣掃描中" : weather ? "天氣已同步" : "等待天氣"}</div><div className={`rounded-xl px-3 py-2 ${routeLoading ? "bg-amber-300/10 text-amber-200" : routes[plans[0]?.title] ? "bg-[#58d9ff]/10 text-[#9deaff]" : "bg-slate-700/30"}`}><span className={`mb-1 block h-1.5 w-1.5 rounded-full ${routeLoading ? "bg-amber-300" : routes[plans[0]?.title] ? "bg-[#58d9ff]" : "bg-slate-500"}`} />{routeLoading ? "路線計算中" : routes[plans[0]?.title] ? "路線已就緒" : "等待路線"}</div></div>
        </div>
      </div>
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 md:px-8 md:py-10">
        <section className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
          <div className="rounded-3xl border border-cyan-300/15 bg-[#0d2030]/90 p-5 shadow-2xl shadow-cyan-950/20 md:p-7">
            <div className="mb-6 flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-widest text-[#67f5c8]">01 / SET THE MOOD</p><h2 className="mt-1 text-2xl font-black">先告訴我你們想怎麼玩</h2><p className="mt-2 text-xs text-slate-500">你的條件只會用來生成這次出遊方案。</p></div><MapPin className="text-[#58d9ff]" /></div>
            ...
            <p className="mb-3 mt-6 text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500">偏好訊號 / VIBE</p><div><p className="mb-2 text-sm text-slate-300">活動風格</p><div className="flex flex-wrap gap-2">{vibes.map((item) => <button key={item} onClick={() => update("vibe", item)} className={`rounded-lg px-3 py-2 text-sm ${form.vibe === item ? "bg-[#58d9ff] font-bold text-[#06131e]" : "bg-[#132d40] text-slate-300"}`}>{item}</button>)}</div></div>
          ...
          <div id="plans" className="space-y-4"><div className="flex items-end justify-between"><div><p className="text-xs font-bold uppercase tracking-widest text-[#58d9ff]">02 / AI SHORTLIST</p><h2 className="mt-1 text-2xl font-black">為你排出的 3 條路線</h2>{routeLoading && <p className="mt-1 text-xs text-cyan-300">正在用 Geoapify 計算路線…</p>}{routeError && <p className="mt-1 text-xs text-amber-300">{routeError}</p>}</div><button onClick={() => setSeed((value) => value + 1)} className="flex items-center gap-1 text-sm text-slate-400 hover:text-[#67f5c8]"><RefreshCw size={15} /> 再生成</button></div>
            {plans.map((plan, planIndex) => <article key={plan.title} className="rounded-3xl border border-slate-700/80 bg-[#102536] p-5 transition hover:-translate-y-0.5 hover:border-[#58d9ff]/60 md:p-6"><div className="flex items-start justify-between gap-4"><div className="flex gap-3"><span className="text-3xl">{plan.emoji}</span><div><div className="flex flex-wrap items-center gap-2"><h3 className="text-xl font-black">{plan.title}</h3>{planIndex === 0 && <span className="rounded-full bg-[#67f5c8] px-2 py-1 text-[10px] font-black uppercase tracking-wider text-[#06131e]">最佳匹配</span>}</div><p className="mt-1 text-sm leading-6 text-slate-300">{plan.summary}</p></div></div><div className="rounded-2xl bg-[#67f5c8]/10 px-3 py-2 text-right"><p className="text-2xl font-black text-[#67f5c8]">{plan.match}%</p><p className="text-[10px] uppercase tracking-wider text-slate-400">match</p></div></div><div className="mt-5 space-y-3 border-l border-[#58d9ff]/40 pl-4">{plan.stops.map((stop, index) => <div key={stop.time} className="relative"><span className="absolute -left-[21px] top-1.5 h-2.5 w-2.5 rounded-full bg-[#58d9ff]" /><div className="flex justify-between gap-3"><p className="text-sm font-bold text-[#67f5c8]">{stop.time}｜{stop.title}</p><span className="text-xs text-slate-400">${stop.cost}</span></div><p className="text-xs text-slate-400">{places[plan.title]?.[index]?.name || stop.detail}</p>{places[plan.title]?.[index]?.address && <p className="mt-1 text-[11px] text-slate-500">{places[plan.title]?.[index]?.address}</p>}</div>)}</div><div className="mt-5 flex flex-wrap gap-2">{plan.tags.map((tag) => <span key={tag} className="rounded-full bg-[#18364a] px-2.5 py-1 text-xs text-[#9deaff]">#{tag}</span>)}</div><div className="mt-5 grid grid-cols-1 gap-2 rounded-2xl bg-[#0a1a28] p-3 text-sm sm:grid-cols-2"><div><p className="text-xs text-slate-500">預估每人</p><p className="font-bold text-white">${plan.cost} <span className={plan.cost <= form.budget ? "text-[#67f5c8]" : "text-rose-300"}>{plan.cost <= form.budget ? "預算內" : "超出"}</span></p></div><div><p className="text-xs text-slate-500">交通</p><p className="font-bold text-white">{routes[plan.title] ? `${routes[plan.title]!.distanceKm} km｜${routes[plan.title]!.durationMinutes} 分鐘` : plan.travel}</p><p className="mt-1 text-[10px] text-slate-500">{routes[plan.title] ? "Geoapify 路線估算" : "等待地點定位"}</p></div></div>{plan.warnings.map((warning) => <p key={warning} className="mt-3 text-xs text-amber-300">⚠ {warning}</p>)}<div className="mt-5 flex flex-col gap-2 sm:flex-row"><button onClick={() => openGroup(plan)} className="flex min-h-12 flex-1 items-center justify-center gap-2 rounded-xl bg-[#58d9ff] px-4 py-3 text-sm font-black text-[#06131e] shadow-lg shadow-cyan-950/30 hover:bg-[#8ae7ff]"><Users size={16} /> 一鍵開團</button><button onClick={() => { setSelectedPlan(plan); setShareOpen(true); }} className="flex min-h-12 items-center justify-center rounded-xl border border-slate-600 px-4 py-3 text-sm font-bold text-slate-200 hover:border-[#67f5c8]"><Copy size={16} /></button></div></article>)}
          </div>
        </section>
      </main>

      {shareOpen && selectedPlan && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4"><div className="w-full max-w-md rounded-3xl border border-cyan-300/20 bg-[#102536] p-6 shadow-2xl"><div className="flex items-center justify-between"><h3 className="text-xl font-black">邀請朋友一起決定</h3><button onClick={() => setShareOpen(false)} className="text-slate-400"><X size={20} /></button></div><p className="mt-3 text-sm text-slate-300">先把「{selectedPlan.title}」分享給朋友，大家可以一起比較方案。</p><div className="mt-4 grid grid-cols-2 gap-2 rounded-2xl bg-[#0a1a28] p-3 text-xs"><div><p className="text-slate-500">活動日期</p><p className="mt-1 font-bold text-white">{form.date || "尚未設定"}</p></div><div><p className="text-slate-500">參加人數</p><p className="mt-1 font-bold text-white">{form.people} 人</p></div>{weather && <div className="col-span-2 border-t border-slate-700/70 pt-2"><p className="text-slate-500">天氣偵測</p><p className="mt-1 font-bold text-[#67f5c8]">{weather.summary}｜{weather.minTemperature}–{weather.maxTemperature}°C｜降雨 {weather.precipitationProbability}%</p></div>}</div><button onClick={() => sharePlan(selectedPlan)} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-[#67f5c8] px-4 py-3 font-black text-[#06131e]"><Copy size={16} /> 複製／分享方案</button><button onClick={() => openGroup(selectedPlan)} className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl border border-[#58d9ff] px-4 py-3 font-bold text-[#58d9ff]"><ArrowRight size={16} /> 確認方案並一鍵開團</button></div></div>}
