...
import { EVENT_TAGS, REGIONS } from "@/lib/constants";
  ...
  return (
    <form onSubmit={handleSubmit} className="card-surface flex flex-col gap-2 rounded-2xl p-2 md:flex-row md:items-center md:gap-0 md:rounded-full">
      ...
      <div className="hidden items-center gap-2 px-3 py-2 lg:flex">
        <SlidersHorizontal size={16} className="text-soft" />
        <select value={tag} onChange={(e) => setTag(e.target.value)} className="max-w-28 bg-transparent text-sm text-main outline-none">
          <option value="">興趣標籤</option>
          {EVENT_TAGS.map((item) => <option key={item} value={item}>#{item}</option>)}
        </select>
      </div>
      <button type="button" onClick={() => router.push("/?#search-results")} className="hidden items-center gap-1.5 rounded-full px-3 py-2 text-sm font-semibold text-soft hover:text-brand-600 md:flex">
