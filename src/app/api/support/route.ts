...
export async function GET() {
  ...
  return NextResponse.json({ announcements, disabledServices: controls.map((item) => item.service), errorCodes: ERROR_CATALOG }, { headers: { "Cache-Control": "no-store, max-age=0" } });
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const report = await db.insert(errorReports).values({ userId: user?.id ?? null, kind, errorCode: kind === "error" ? String(body.errorCode) : null, errorName, pagePath: sanitize(String(body.pagePath || "/"), 300), email: sanitize(String(body.email || user?.email || ""), 255) || null, message }).returning({ id: errorReports.id });
    let mailWarning = false;
    try { const result = await sendSupportMail(`[JoinJoy] ${kind === "error" ? `${body.errorCode} ${errorName}` : kind}`, `報告編號：${report[0]?.id}\n使用者：${user?.email || body.email || "未登入"}\n頁面：${body.pagePath || "/"}\n\n${message}`); mailWarning = !result.sent; } catch { mailWarning = true; }
    return NextResponse.json({ ok: true, id: report[0]?.id, mailWarning });
  } catch (error) {
    const raw = error instanceof Error ? error.message : String(error);
    const code = /^[A-Z]+-\d+$/.test(raw) ? raw : null;
    return NextResponse.json({ ok: false, error: code ? "抱歉，遇到了一些錯誤。" : raw || "抱歉，遇到了一些錯誤。", code }, { status: 400 });
  }
}
