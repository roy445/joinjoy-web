...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!rateLimit(clientKey(req, `verify-code-${user.id}`), 10, 10 * 60 * 1000)) {
    ...
    const [record] = await db
      .select()
      .from(oneTimeCodes)
      .where(and(eq(oneTimeCodes.code, code), eq(oneTimeCodes.revoked, false), isNull(oneTimeCodes.usedBy)))
      .limit(1);
    ...
    return NextResponse.json({ ok: true, message: "驗證成功，你已獲得 1 次建立活動的權限！" });
  } catch (err) {
